...
app.post('/api/usuario', (req, res) => {
  ...
  if (!rut || !nombre || !contraseña || !email || !telefono) {
    return res.status(400).json({ error: 'Faltan campos requeridos' });
  }
...
app.post('/api/reservas', (req, res) => {
  ...
  if (!ID_User || !ID_Horario) {
    return res.status(400).send('Faltan campos requeridos');
  }

  // Generar automáticamente la fecha de creación en formato ISO si no se envía
  const FechaCreacion = new Date().toISOString().slice(0, 10);

  // Primero, obtener el email del usuario usando el ID_User
  ...
  connection.query(queryUsuario, [ID_User], (err, userResults) => {
    ...
    // Verificar si se encontró un usuario con ese ID_User
    ...
    // Luego, obtener el email del médico relacionado con el ID_Horario
    const queryMedico = `
    ...
    connection.query(queryMedico, [ID_Horario], (err, medicoResults) => {
      ...
      // Verificar si se encontró un médico con ese ID_Horario
      ...
      connection.query(queryReserva, [ID_User, ID_Horario, FechaCreacion], (err, results) => {
        ...
        // Enviar correos después de la creación exitosa de la reserva
